...
func X__floatscan(tls *TLS, f uintptr, prec int32, pok int32) float64 {
	...
	for i = size_t(0); i < size_t(8) && c|32 == int32(*(*uint8)(unsafe.Pointer(ts + uintptr(i)))); i++ {
		if i < size_t(7) {
			c = func() int32 {
				if (*FILE)(unsafe.Pointer(f)).rpos != (*FILE)(unsafe.Pointer(f)).shend {
					return int32(*(*uint8)(unsafe.Pointer(PostIncUintptr(&(*FILE)(unsafe.Pointer(f)).rpos, 1))))
	...
	if !(i != 0) {
		for i = size_t(0); i < size_t(3) && c|32 == int32(*(*uint8)(unsafe.Pointer(ts + 9 + uintptr(i)))); i++ {
			if i < size_t(2) {
				c = func() int32 {
					if (*FILE)(unsafe.Pointer(f)).rpos != (*FILE)(unsafe.Pointer(f)).shend {
						return int32(*(*uint8)(unsafe.Pointer(PostIncUintptr(&(*FILE)(unsafe.Pointer(f)).rpos, 1))))
...
type __sigset_t = struct{ __bits [32]uint32 }

type sigset_t = __sigset_t

type pthread_attr_t = struct{ __u struct{ __i [9]int32 } }

type pthread_mutex_t = struct{ __u struct{ __i [6]int32 } }

type pthread_cond_t = struct{ __u struct{ __i [12]int32 } }

type pthread_rwlock_t = struct{ __u struct{ __i [8]int32 } }

type pthread_barrier_t = struct{ __u struct{ __i [5]int32 } }
...
func a_clz_32(tls *TLS, x uint32_t) int32 {
...
type a_cas_p_undefined_but_pointer_not_32bit = [1]uint8
...
func __pthread_self(tls *TLS) pthread_t {
	var p uintptr_t
...
func X__inet_aton(tls *TLS, s0 uintptr, dest uintptr) int32 {
	...
	var s uintptr = s0
	var d uintptr = dest
	*(*[4]uint32)(unsafe.Pointer(bp)) = [4]uint32{0: uint32(0)}
...
__110:
	...
	switch Xmbrtowc(tls, bp+260, func() uintptr { *(*uint8)(unsafe.Pointer(bp + 264)) = uint8(c); return bp + 264 }(), uint32(1), bp+268) {
...
__112:
	...
	*(*wchar_t)(unsafe.Pointer(wcs + uintptr(PostIncUint32(&i, 1))*4)) = *(*wchar_t)(unsafe.Pointer(bp + 260))
__115:
	;
	if !(alloc != 0 && i == k) {
		goto __116
...
func Xstrtoul(tls *TLS, s uintptr, p uintptr, base int32) uint32 {
	if __ccgo_strace {
		trc("tls=%v s=%v p=%v base=%v, (%v:)", tls, s, p, base, origin(2))
	}
...
func Xstrtol(tls *TLS, s uintptr, p uintptr, base int32) int32 {
	if __ccgo_strace {
		trc("tls=%v s=%v p=%v base=%v, (%v:)", tls, s, p, base, origin(2))
	}
