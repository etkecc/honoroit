...
type X__vector128 = struct{ Fu [4]X__u32 }
...
type X__kernel_sigset_t = uint64
...
type X_s390_regs_common = struct {
	Fpsw  X_psw_t
	Fgprs [16]uint64
	Facrs [16]uint32
}

type X_s390_fp_regs = struct {
	Ffpc  uint32
	Fpad  uint32
	Ffprs [16]float64
}

type X_sigregs = struct {
	Fregs   X_s390_regs_common
	Ffpregs X_s390_fp_regs
}

type X_sigregs_ext = struct {
	Fvxrs_low   [16]uint64
	Fvxrs_high  [16]X__vector128
	F__reserved [128]uint8
}
...
type Gregset_t = [27]Greg_t

type Fpreg_t = struct{ Fd float64 }
